...
public class ReservationController {
    ...
    @GetMapping("/mine") @PreAuthorize("hasRole('PLAYER')") public List<ReservationResponse> mine(Authentication a){return service.mine(current.require(a));}
    @GetMapping("/owner-agenda") @PreAuthorize("hasAnyRole('OWNER','ADMIN')") public List<ReservationResponse> agenda(@RequestParam @DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME) OffsetDateTime from,@RequestParam @DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME) OffsetDateTime to,Authentication a){return service.ownerAgenda(current.require(a),from,to);}
